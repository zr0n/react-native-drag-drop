...
class DragModal extends React.Component {
  render() {
    ...
    let {highlightZone} = this.props
    return (
      <Modal
        onRequestClose={() => false}
        transparent={true}
        supportedOrientations={allOrientations}>
          <DropZoneHighlight zone={highlightZone}/>
...
class DropZoneHighlight extends React.Component {
  render(){
...
class DragContainer extends React.Component {
    ...
    _handleDragging(point) {
      ...
      this.dropZones.forEach((zone) => {
        const hlZone = this.state.highlightZone
        let isHighlight = false
        if(hlZone && hlZone.x == zone.x && hlZone.y == zone.y){
          isHighlight = true
        }
...
//        onMoveShouldSetPanResponderCapture: (evt, gestureState) => true,
    ...
    render() {
        return <View style={[{flex: 1}, this.props.style]} onLayout={e => this.containerLayout = e.nativeEvent.layout} {...this._panResponder.panHandlers}>
          {this.props.children}
          {this.state.draggingComponent ? <DragModal highlightZone={this.state.highlightZone} content={this.state.draggingComponent} location={this.state.location} drop={this._handleDrop} /> : null}
        </View>;
    }
}
