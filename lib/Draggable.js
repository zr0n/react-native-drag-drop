        ...
        return <TouchableOpacity activeOpacity={this.props.activeOpacity} style={this.props.style} onLongPress={onLongPress}  onPress={this.props.onPress} onPressIn={onPressIn} ref="wrapper">
        {
          React.Children.map(this.props.children, child => {
            ...
            if( clickables.indexOf(childType) > -1 ){ //if child has onLongPress and onPressIn props
              const onLongPressChild = childClone.props.onLongPress
              const onPressInChild = childClone.props.onPressIn
              const onLongPressOverriden = () => {
                if(typeof onLongPress === 'function'){
                  onLongPress()
                }
                if(typeof onLongPressChild === 'function'){
                  onLongPressChild()
                }
              }
              const onPressInOverriden = () => {
                if(typeof onPressIn === 'function'){
                  onLongPressChild()
                }
                if(typeof onPressInChild === 'function'){
                  onPressInChild()
                }
              }
              childClone.props = Object.assign({}, childClone.props, { //give it the parent props
                onLongPress: onLongPressOverriden,
                onPressIn: onPressInOverriden
              })
            }
